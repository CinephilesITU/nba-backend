...
# ---------------------------------------------------------
# GLOBAL İSTATİSTİK TANIMLARI (Frontend Tooltip'leri için)
# ---------------------------------------------------------
STAT_DEFINITIONS = {
    "PTS": "Points Per Game (The primary scoring metric.)",
    "AST": "Assists Per Game (Passes leading to a direct score.)",
    "REB": "Rebounds Per Game (Total offensive and defensive rebounds.)",
    "Efficiency": "Player Efficiency Rating (PER). A comprehensive metric measuring a player's overall contribution.",
    "TOV": "Turnovers (The number of times a player loses possession.)",
    "FG_PCT": "Field Goal Percentage (Two and three-point shot success rate.)",
    "STL": "Steals (Defensive turnovers created.)",
...
# E. TAKIM ARAMA (SEARCH) - YENİ EKLENDİ
@app.route('/api/v1/teams/search', methods=['GET'])
def search_teams():
    conn = get_db_connection()
    if not conn: return jsonify({"error": "DB Baglantisi Yok"}), 500
    cursor = conn.cursor(dictionary=True)
    ...
    # Takım Adı veya Kısaltma ile arama
    sql = """
        SELECT * 
        FROM TEAMS 
        WHERE teamName LIKE %s OR teamAbbreviation LIKE %s 
        LIMIT 10
    """
    search_term = f"%{query}%"
...
# F. TAKIM MAÇ PROGRAMI (FIXTURES) - YENİ
